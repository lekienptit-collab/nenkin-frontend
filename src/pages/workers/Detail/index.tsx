...
const withError = async (action: () => Promise<void>) => {
  try {
    ...
    message.error('Không mở được file PDF. Xin thử lại!');
  }
};
  ...
  >
    {!procedure ? (
      <Empty
        ...
        description="Chưa làm thủ tục lần này"
      />
    ) : (
      <>
        <ProDescriptions column={{ xs: 1, md: 3 }} size="small" style={{ marginBottom: 12 }}>
          <ProDescriptions.Item label="Người được uỷ quyền">
            {procedure.agent?.name || '-'}
          </ProDescriptions.Item>
          <ProDescriptions.Item label="Quan hệ">
            {procedure.relation || '-'}
          </ProDescriptions.Item>
          {serviceType === SERVICE_TYPE.FIRST ? (
            <>
              <ProDescriptions.Item label="Ngày làm đơn">
                {formatDate(procedure.requestDate)}
              </ProDescriptions.Item>
              <ProDescriptions.Item label="Ngày uỷ quyền">
                {formatDate(procedure.entrustDate)}
              </ProDescriptions.Item>
            </>
          ) : (
            <>
              <ProDescriptions.Item label="Ngày làm đơn khai thuế">
                {formatDate(procedure.taxRequestDate)}
              </ProDescriptions.Item>
              <ProDescriptions.Item label="Ngày uỷ quyền khai thuế">
                {formatDate(procedure.taxEntrustDate)}
              </ProDescriptions.Item>
              <ProDescriptions.Item label="Văn phòng thuế">
                {procedure.taxOffice || '-'}
              </ProDescriptions.Item>
            </>
          )}
            ...
            },
            {
              ...
              render: (_, record) =>
                record.status === 'GENERATED' ? (
                  ...
                  <Tag color="default">Chưa có file PDF</Tag>
                ),
            },
          ]}
        />
...
const WorkerDetail: React.FC = () => {
  ...
  const infoTab = (
    <>
      <ProCard title="Thông tin cá nhân - hộ chiếu" bordered style={{ marginBottom: 16 }}>
        <ProDescriptions column={{ xs: 1, md: 3 }}>
          <ProDescriptions.Item label="Họ và tên">{worker?.name}</ProDescriptions.Item>
          <ProDescriptions.Item label="Giới tính">
            {worker?.gender === undefined ? '-' : GENDER_LABELS[worker.gender]}
          </ProDescriptions.Item>
          <ProDescriptions.Item label="Ngày tháng năm sinh">
            {formatDate(worker?.dateOfBirth)}
          </ProDescriptions.Item>
          <ProDescriptions.Item label="Số điện thoại">
            {worker?.phoneNumber || '-'}
          </ProDescriptions.Item>
          <ProDescriptions.Item label="Quốc tịch">
            {worker?.country || '-'}
          </ProDescriptions.Item>
          <ProDescriptions.Item label="Ngày rời Nhật Bản">
            {formatDate(worker?.leaveJapanDate)}
          </ProDescriptions.Item>
        </ProDescriptions>
        <Row gutter={16}>
          <Col xs={24} md={8}>
            <div>Ảnh hộ chiếu (trang đầu)</div>
            <ImageUploader readOnly value={worker?.passportFirstPage} />
          </Col>
          <Col xs={24} md={8}>
            <div>Ảnh hộ chiếu (trang hai)</div>
            <ImageUploader readOnly value={worker?.passportSecondPage} />
          </Col>
          <Col xs={24} md={8}>
            <div>Ảnh hộ chiếu (trang có dấu)</div>
            <ImageUploader readOnly value={worker?.passportStampPage} />
          </Col>
        </Row>
      </ProCard>

      <ProCard title="Thông tin địa chỉ" bordered style={{ marginBottom: 16 }}>
        <ProDescriptions column={{ xs: 1, md: 2 }}>
          <ProDescriptions.Item label="Tỉnh (hiện tại)">
            {findLabel(master?.vnProvinces, worker?.addressVnPrefectureCode)}
          </ProDescriptions.Item>
          <ProDescriptions.Item label="Thành phố/Huyện">
            {worker?.addressVnDistrict || '-'}
          </ProDescriptions.Item>
          <ProDescriptions.Item label="Mã bưu điện (Việt Nam)">
            {worker?.addressVnPostalCode || '-'}
          </ProDescriptions.Item>
          <ProDescriptions.Item label="Địa chỉ tại Việt Nam">
            {worker?.addressVnAddress || '-'}
          </ProDescriptions.Item>
          <ProDescriptions.Item label="Mã bưu điện (Nhật Bản)">
            {worker?.addressJpPostalCode || '-'}
          </ProDescriptions.Item>
          <ProDescriptions.Item label="Địa chỉ cuối cùng ở Nhật Bản">
            {[
        ...
        <div>Giấy tờ chứng minh đã rời Nhật Bản</div>
        <ImageUploader readOnly value={worker?.leftProofUrl} />
      </ProCard>

      <ProCard title="Thông tin thẻ ngoại kiều" bordered style={{ marginBottom: 16 }}>
        ...
        <Row gutter={16}>
          <Col xs={24} md={12}>
            <div>Ảnh thẻ ngoại kiều (mặt trước)</div>
            <ImageUploader readOnly value={worker?.residenceCardFrontImage} />
          </Col>
          <Col xs={24} md={12}>
            <div>Ảnh thẻ ngoại kiều (mặt sau)</div>
            <ImageUploader readOnly value={worker?.residenceCardBackImage} />
          </Col>
        </Row>
      </ProCard>

      <ProCard title="Thông tin sổ Nenkin" bordered style={{ marginBottom: 16 }}>
        <ProDescriptions column={{ xs: 1, md: 2 }}>
          <ProDescriptions.Item label="Mã số lương hưu cơ sở">
            {worker?.pensionNumber || '-'}
          </ProDescriptions.Item>
          <ProDescriptions.Item label="Họ và tên (Katakana)">
            {worker?.nameFurigana || '-'}
          </ProDescriptions.Item>
        </ProDescriptions>
        <div>Ảnh chụp sổ Nenkin (trang đầu)</div>
        <ImageUploader readOnly value={worker?.nenkinBookImage} />
      </ProCard>

      <ProCard title="Thông tin tài khoản Ngân hàng" bordered style={{ marginBottom: 16 }}>
        <ProDescriptions column={{ xs: 1, md: 2 }}>
          <ProDescriptions.Item label="Quốc gia">
          ...
          <ProDescriptions.Item label="Mã Swift (BIC)">
            {worker?.bankSwiftCode || '-'}
          </ProDescriptions.Item>
          <ProDescriptions.Item label="Địa chỉ chi nhánh">
          ...
          <ProDescriptions.Item label="Tên tài khoản (Katakana)">
        ...
        <Row gutter={16}>
          <Col xs={24} md={12}>
            <div>Giấy xác nhận tài khoản ngân hàng</div>
            <ImageUploader readOnly value={worker?.bankImage} />
          </Col>
          <Col xs={24} md={12}>
            <div>Giấy xác nhận tài khoản ngân hàng (trang hai)</div>
            <ImageUploader readOnly value={worker?.bankImageBack} />
          </Col>
        </Row>
      </ProCard>

      <ProCard title="Thông tin thuế" bordered style={{ marginBottom: 16 }}>
        <ProDescriptions column={{ xs: 1, md: 3 }}>
          <ProDescriptions.Item label="Số tiền được miễn đánh thuế">
            {formatYen(worker?.taxDeduct)}
          </ProDescriptions.Item>
          <ProDescriptions.Item label="Số tiền thuế phải nộp">
            {formatYen(worker?.taxAmount)}
          </ProDescriptions.Item>
          <ProDescriptions.Item label="Số tiền bảo hiểm hưu trí thực lĩnh">
      ...
      <ProCard
        title="Quá trình tham gia chế độ lương hưu chung"
        bordered
        style={{ marginBottom: 16 }}
      >
        <Table<API.InsuranceHistoryItem>
          ...
          columns={[
            { title: 'Tên cơ sở kinh doanh', dataIndex: 'workPlace' },
            { title: 'Địa chỉ', dataIndex: 'address' },
            {
              title: 'Làm việc từ ngày',
              dataIndex: 'fromDate',
              width: 150,
              render: (value) => formatDate(value),
            },
      ...
      <ProCard title="Thông tin kết quả Nenkin" bordered>
        <ProDescriptions column={{ xs: 1, md: 2 }}>
          <ProDescriptions.Item label="Kết quả Nenkin lần 1">
            <Badge
              status={
          ...
          <ProDescriptions.Item label="Kết quả Nenkin lần 2">
            <Badge
              status={
  ...
  return (
    <PageContainer
      title={worker?.name || 'Chi tiết người lao động'}
      loading={loading}
      onBack={() => history.back()}
      extra={
    ...
    >
      {missingLabels.length > 0 && (
        <Alert
          ...
          showIcon
          style={{ marginBottom: 16 }}
          message="Thông tin về người lao động chưa đầy đủ"
          description={`Còn thiếu: ${missingLabels.join(', ')}.`}
          action={
            checkAccess.updateWorker && (
              <Space>
                <Link to={`/workers/${workerId}/edit`}>Bổ sung thông tin</Link>
              </Space>
            )
          }
        />
      ...
      <ProCard
        tabs={{
          items: [
            ...
            { key: 'papers', label: 'Giấy tờ đã làm', children: paperTab },
          ],
        }}
      />
    </PageContainer>
